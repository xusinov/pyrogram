    ...
    def _eval_type(self, globalns, localns):
        if not self.__forward_evaluated__ or localns is not globalns:
            ...
            self.__forward_value__ = _type_check(
                eval(self.__forward_code__, globalns, localns), "Forward references must evaluate to types."
            )
...
class TypeVar(_TypingBase, _root=True):
    ...
    def __init__(self, name, *constraints, bound=None, covariant=False, contravariant=False):
        super().__init__(name, *constraints, bound=bound, covariant=covariant, contravariant=contravariant)
        self.__name__ = name
...
class GenericMeta(TypingMeta, abc.ABCMeta):
    ...
    * if Generic[...] is present, then it should list all type variables
    ...
    def __new__(cls, name, bases, namespace, tvars=None, args=None, origin=None, extra=None, orig_bases=None):
            ...
            if gvars is None:
                ...
                if not tvarset <= gvarset:
                    raise TypeError(
                        ...
                        % (", ".join(str(t) for t in tvars if t not in gvarset), ", ".join(str(g) for g in gvars))
                    )
                tvars = gvars
        ...
        self.__args__ = (
            tuple(... if a is _TypingEllipsis else () if a is _TypingEmpty else a for a in args) if args else None
        )
    ...
    def __setattr__(self, attr, value):
        ...
        if (
            ...
            or self._gorg is None  # The class is not fully created, see #typing/506
        ):
        ...
        for base in protocol_bases:
            for attr in base.__dict__.keys():
                ...
                for c in self.__mro__:
                    ...
                    if (
                        not attr.startswith("_abc_")
                        and attr != "__abstractmethods__"
                        and attr != "__annotations__"
                        and attr != "__weakref__"
                        and attr != "_is_protocol"
                        and attr != "_gorg"
                        and attr != "__dict__"
                        and attr != "__args__"
                        and attr != "__slots__"
                        and attr != "_get_protocol_attrs"
                        and attr != "__next_in_mro__"
                        and attr != "__parameters__"
                        and attr != "__origin__"
                        and attr != "__orig_bases__"
                        and attr != "__extra__"
                        and attr != "__tree_hash__"
                        and attr != "__module__"
                    ):
                        attrs.add(attr)
...
class ItemsView(
    MappingView[Tuple[KT, VT_co]], AbstractSet[Tuple[KT, VT_co]], Generic[KT, VT_co], extra=collections_abc.ItemsView
):
    __slots__ = ()
